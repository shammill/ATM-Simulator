...
namespace ATM_Simulation {
    class ATM {
        ...
        static string ShowMainMenu() {
            Console.Clear();
            Console.WriteLine("--------------------------------------------");
            Console.WriteLine("Welcome to Sams Easy ATM.\n");
        ...
        static void AccountsMenu(string mainMenuOptionSelected) {
            switch (mainMenuOptionSelected) {
                case "0": ExitATM(); //0. Exit Program
                    break;
                case "1": DisplayAccountsMenu(1); //1. Check Balance
                    AccountsActionMenu(1);
                    break;
                case "2": DisplayAccountsMenu(2); //2. Withdrawal 
                    AccountsActionMenu(2);
                    break;
                case "3": DisplayAccountsMenu(3); //3. Transfer
                    AccountsActionMenu(3);
                    break;
                default: Console.Write("Incorrect selection, Please Try Again: 0 ... 3: ");
                    break;
            }
        ...
        static void AccountsActionMenu(int mainMenuOptionSelected) {
            ...
            while (isRunning == true) {
                ...
                switch (mainMenuOptionSelected) {
                    case 1: DisplayBalance(whichAccount);
                        isRunning = UserInputYesNo(mainMenuOptionSelected);
                        break;
                    case 2: WithdrawAmount(whichAccount);
                        isRunning = UserInputYesNo(mainMenuOptionSelected);
                        break;
                    case 3: TransferPromtMenu(whichAccount);
                        isRunning = UserInputYesNo(mainMenuOptionSelected);
                        break;
                }
            }
        ...
        static double UserTransactionCashAmount(int whichAccount, int transactionType) {
            ...
            while (isRunning == true) {
                ...
                if (isGoodNumber && (userCashInput == 0)) { // If input is 0 stop running
                    isRunning = false;
                } 
                else if (isGoodNumber && userCashInput > 0 ) {
                    if ((whichAccount == 3) && ((accountBalances[whichAccount] - userCashInput) < -2000)) {// Overdrawing the credit account
                    Console.Write("\nYou cannot be more than $2000 in debt on your credit account. \nPlease try another amount or 0 to cancel: ");
                    } 
                    else if ((whichAccount == 3) && ((accountBalances[whichAccount] - userCashInput) >= -2000)) { // Credit Withdraw
                        isRunning = false;
                        return userCashInput;
                    } 
                    else if ((userCashInput > accountBalances[whichAccount])) { // Not enough in account for transaction
                        Console.Write("\nSorry, there are insufficent funds on the account. \nPlease try another amount or 0 to cancel:  ");
                    } 
                    else if ((userCashInput <= accountBalances[whichAccount] && ((accountBalances[whichAccount] - userCashInput) < transactionType))) { //Normal Account overdraw 
                        Console.Write("\nSorry, there are insufficent funds on the account. \nPlease try another amount or 0 to cancel: ");
                    } else if ((userCashInput <= accountBalances[whichAccount] && ((accountBalances[whichAccount] - userCashInput) >= transactionType))) { //Normal Account withdraw 
                        isRunning = false;
                        return userCashInput;
                    }
                }
        ...
        static void UserInputAccountsMenu(out int userInput) {
            ...
            while (isRunning == true) {
                ...
                if (isGoodNumber && (userInput == 0 || userInput == 1 || userInput == 2 || userInput == 3 || userInput == 4)) {
                    ...
                    Console.Write("Incorrect Selection. Please Try Again: 0 ... 4: ");
                }
            }
        }//end UserInputAccountsMenu
            ...
            while (validInput == false) {
                ...
                switch (userChoice) {
                    ...
                    case "n": validInput = true;
                        Console.Clear();
                        return false;
                    default: Console.Write("Incorrect selection, Please Try Again: <Y or N> ");
                        break;
                }
            }
            return false;
        ...
        static void ExitATM() {
            Console.Clear();
            Console.WriteLine("\n\tThank you for using Sams Easy ATM, have a great day!");
